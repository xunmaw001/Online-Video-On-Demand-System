            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">视频收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="dianyingName" id="dianyingName" placeholder="视频名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                                                                                                     
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              <label class="layui-form-label">视频类型名称</label>
                              <div class="layui-input-inline">
                                  <select name="leixTypes" id="leixTypes" lay-filter="leixTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in leixTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../dianying/detail.jsp?id='+item.dianyingId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.dianyingPhoto?item.dianyingPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../dianying/detail.jsp?id='+item.dianyingId)"><span>{{item.dianyingName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.dianyingNewMoney"><span style="color: red;">{{item.dianyingNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteDianyingCollection(item.id)">删除 {{item.dianyingName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 dianyingCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            dianyingCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteDianyingCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('dianyingCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#dianyingName').val() != null && jquery('#dianyingName').val() != "" && jquery('#dianyingName').val() != "null" ) {
                param['dianyingName'] = jquery('#dianyingName').val();
            }
                                                                                                     
            var leixTypes = $("#leixTypes").find("option:selected").val();
            if (leixTypes != null && leixTypes != "" && leixTypes != "null") {
                            param['leixTypes'] = leixTypes;
                        }
                                        
            // 当前表
                                                    // 获取列表数据
            http.request('dianyingCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function leixTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=leix_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.leixTypesList = res.data.list;
               }
           });
       }
